...
%         Rc_peak=max(Rc(10000:10500))-min(Rc(10000:10500));
        ...
        Rc_peak=max(abs(Rc(10000:10500)));

%         Rc_steady=abs(Rc_base-min(Rc(10000:10500)));
     

        Rc_steady=abs(mean(Rc(end-10000:end)));
...
%         current_of_interest=medfilt1(current_of_interest,100);
        ...
        current_of_interest_waves(trial_number,:)=medfilt1(current_of_interest,1);
        
        
        current_of_interest_waves_means=movmean(current_of_interest_waves,1);
        ...
        [current_amp_first,lk_trace,width_trace, spike_amp_trace]=findpeaks(medfilt1(current_of_interest(10:end),10),'MinPeakHeight',20,'MinPeakDistance',10,'MinPeakWidth', 50);
        ...
        current_amp_all_sweeps_abs_max(trial_number)=max(medfilt1(current_of_interest,10));

        charge_first_allwaves(trial_number)=trapz(1/sampling_rate,medfilt1(current_of_interest(1:end),1));
...
%         baseline_post=mean(current_of_interest(3001:end));
        ...
        if isempty(current_amp_first)==0
            ...
            if isempty(current_decay)==1
               ...
               current_amp_all_sweeps_max(trial_number)=max(current_amp_first);
               

               current_time_all_sweeps_first(trial_number)=((lk_trace(1))*1000)/sampling_rate;
               all_currents_amp{trial_number}=current_amp_first;
               all_currents_time{trial_number}=((lk_trace)*1000)/sampling_rate;
%                 all_currents_time{trial_number}=lk_trace-1000;
